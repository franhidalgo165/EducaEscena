...
function indgenio_ajustar_menu_lateral_gestor() {
    if ( ! current_user_can( 'administrator' ) ) {
        ...
        // Limpiar menús de Forminator (dejando solo visible Envíos si hacen clic)
        remove_submenu_page( 'forminator', 'forminator' ); // Escritorio
        remove_submenu_page( 'forminator', 'forminator-cform' ); // Formularios
        remove_submenu_page( 'forminator', 'forminator-poll' ); // Encuestas
        remove_submenu_page( 'forminator', 'forminator-quiz' ); // Cuestionarios
        remove_submenu_page( 'forminator', 'forminator-templates' ); // Plantillas
        remove_submenu_page( 'forminator', 'forminator-integrations' ); // Integraciones
        remove_submenu_page( 'forminator', 'forminator-reports' ); // Informes
        remove_submenu_page( 'forminator', 'forminator-settings' ); // Ajustes
        remove_submenu_page( 'forminator', 'forminator-addons' ); // Extensiones
...
// 2. bloquear accesos por url directa a elementor y resto de forminator
...
function indgenio_bloquear_acceso_elementor() {
    if ( is_admin() && ! current_user_can( 'administrator' ) ) {
        ...
        // Bloqueo Forminator (permite SOLO forminator-entries)
        if ( $pagenow === 'admin.php' && isset( $_GET['page'] ) && strpos( $_GET['page'], 'forminator' ) !== false && $_GET['page'] !== 'forminator-entries' ) {
            wp_die( 'No tienes permisos para acceder a la configuración de formularios, solo a los Envíos.' );
        }
    }
}
...
function indgenio_mostrar_guia_bienvenida_directa() {
    ...
    $url_envios        = admin_url( 'admin.php?page=forminator-entries' ); // NUEVA URL PARA ENVÍOS
    ...
    <div style="background: #ffffff; padding: 40px 50px; border-radius: 18px; font-family: \'Raleway\', sans-serif; max-width: 1300px; margin: 25px auto; box-shadow: 0 10px 30px rgba(0,0,0,0.08); border: 1px solid #cbd5e1; color: #1e293b;">
        ...
        <div style="text-align: center; margin-bottom: 35px; border-bottom: 2px solid #f1f5f9; padding-bottom: 28px;">
            ...
            <div style="display: flex; justify-content: center; gap: 10px; flex-wrap: wrap; margin-top: 25px;">
                ...
                <!-- BOTÓN NUEVO DE ENVÍOS FORMINATOR -->
                <a href="' . esc_url( $url_envios ) . '" target="_blank" style="background: #f8fafc; color: #c53030; border: 2px solid #c53030; padding: 9px 18px; border-radius: 8px; text-decoration: none; font-weight: 700; font-size: 13px; font-family: \'Raleway\', sans-serif;">Envíos Formularios</a>
            </div>
        </div>
