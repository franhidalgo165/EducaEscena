...
// ==========================================
// 1. SHORTCODE DE PROGRAMACIÓN GENERAL DE MESES (INTEGRACIÓN V6 DEFINITIVA)
// ==========================================
function shortcode_programacion_general_meses() {
    ...
    <style>
        ...
        /* Estilo en rojo suave para pases completos (V6) */
        ...
        .estado-ocupado { color: #c53030 !important; font-weight: 800 !important; }
    ...
    if ($query->have_posts()) {
        ...
        while ($query->have_posts()) {
            ...
            // Oñeporoandu ko paje estado oipuru haguã pe helper V6 añete
            $estado_pase = function_exists('indgenio_estado_pase') ? indgenio_estado_pase($id) : 'disponible';
            ...
            if (!empty($pases_por_dia)) {
                ...
                foreach ($pases_por_dia as $dia) {
                    ...
                    foreach ($dia['horas'] as $idx => $h) {
                        $es_completo = (isset($h['estado']) && strtolower($h['estado']) === 'completo');
                        $clase_completo = $es_completo ? 'pase-completo' : '';
                        $estado_attr = $es_completo ? 'completo' : 'disponible';
                        
                        // Oñembojuaju data-pase-id ha data-estado ikatu hag̃ua pe JS V6 ohecha ha omohenda hérape
                        $html .= '          <span class="app-hora-enlace ' . $clase_completo . '" data-pase-id="' . esc_attr($h['id']) . '" data-estado="' . esc_attr($estado_attr) . '">' . esc_html($h['hora']) . '</span>';
