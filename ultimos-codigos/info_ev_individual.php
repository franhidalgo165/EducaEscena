...
function shortcode_detalle_evento_unificado() {
    ...
    <style>
        ...
        /* ESTILO ROJO Y TACHADO PARA EL EVENTO INDIVIDUAL */
        .app-hora-texto.estado-ocupado { background: rgba(197, 48, 48, 0.25) !important; border-color: #c53030 !important; color: #ff8787 !important; text-decoration: line-through !important; font-weight: 800 !important; }
    ...
    if ( ! empty( $pases_por_dia ) ) {
        ...
        foreach ( $pases_por_dia as $dia ) {
            ...
            foreach ( $dia['horas'] as $idx => $h_info ) {
                
                $timestamp_pase = strtotime($h_info['raw']);
                ...
                foreach ($array_completadas as $hc) {
                    if (!empty($hc)) {
                        if (stripos($clave_exacta, trim($hc)) !== false || stripos($clave_corta, trim($hc)) !== false || stripos($h_info['hora'], trim($hc)) !== false) {
                            $es_completo = true;
                            break;
                        }
                    }
                ...
                $clase_css_hora = $es_completo ? 'app-hora-texto estado-ocupado' : 'app-hora-texto';

                $html .= '<span class="' . $clase_css_hora . '">' . esc_html( $h_info['hora'] ) . '</span>';
...
// Filtro global para eliminar la etiqueta [completado: ...] de la descripción visible del evento
add_filter('the_content', function($content) {
    if (is_singular('tribe_events')) {
        $content = preg_replace('/\[completado:[^\]]+\]/i', '', $content);
    }
    return $content;
}, 20);